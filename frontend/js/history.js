      ...
      <div style="display: flex; align-items: center; gap: 0.5rem;">
        ...
        <div style="flex:1;overflow:hidden;">
          <div style="display:flex;align-items:center;gap:0.3rem;">
            <span style="font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1;">${p.emoji} ${escapeHtml(conv.title)}</span>
            <button class="rename-btn" data-id="${conv.id}" title="Renommer" style="background:none;border:none;cursor:pointer;font-size:0.8rem;opacity:0.5;flex-shrink:0;padding:0 2px;">✏️</button>
          </div>
          <div style="font-size:0.7rem;opacity:0.6;">${formatConversationDate(conv.createdAt)}</div>
        </div>
      </div>
    `;
